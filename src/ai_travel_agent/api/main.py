...
async def ws_plan(websocket: WebSocket) -> None:
    ...
    try:
        ...
        if is_refine:
            ...
            session = _sessions.get(session_id)
            if not session:
                await websocket.send_json({"type": "error", "message": "session required for refine"})
                return
            ...
            _sessions[session_id] = {"raw_input": raw_input, "itinerary": None, "destination": destination}
            bind_session(session_id=session_id, destination=destination)
        ...
        airport_map = {"JFK": "New York (JFK)", "DPS": "Bali (DPS)", "LHR": "London (LHR)", "CDG": "Paris (CDG)", "HND": "Tokyo (HND)", "BOM": "Mumbai (BOM)", "AMD": "Ahmedabad (AMD)", "DEL": "Delhi (DEL)"}
        flights_clean = []
