...
class ConflictDetector:
    ...
    def _impossible_travel(self, day: DayPlan) -> list[Conflict]:
        ...
        for a, b in zip(acts, acts[1:]):
            if not (a.end_time and b.start_time):
                continue
            gap = (
                datetime.combine(day.date, b.start_time)
            ...
            if gap < needed:
                out.append(
                    Conflict(
                        ConflictType.IMPOSSIBLE_TRAVEL,
                        Severity.AUTO_FIXABLE,
                        day.day_number,
    ...
    def _opening_hours(self, day: DayPlan) -> list[Conflict]:
        ...
        for a in day.activities:
            if a.open_time and a.start_time and a.start_time < a.open_time:
                out.append(
                    Conflict(
                        ConflictType.OPENING_HOURS,
                        Severity.AUTO_FIXABLE,
                        day.day_number,
                        f"'{a.title}' scheduled at {a.start_time} but opens at {a.open_time}",
                        [a.attraction_id or ""],
                        {"required_start": a.open_time.isoformat()},
                    )
                )
            if a.close_time and a.end_time and a.end_time > a.close_time:
                out.append(
                    Conflict(
                        ...
                        [a.attraction_id or ""],
                        {"required_end": a.close_time.isoformat()},
                    )
                )
        return out
    ...
    def _meal_gaps(self, day: DayPlan) -> list[Conflict]:
        ...
        if not any(
            self.lunch_window[0] <= (m.start_time or time(0)) <= self.lunch_window[1]
            for m in meals
        ):
            out.append(
                Conflict(
                    ConflictType.MEAL_GAP,
                    Severity.AUTO_FIXABLE,
                    day.day_number,
        ...
        if not any(
            self.dinner_window[0] <= (m.start_time or time(0)) <= self.dinner_window[1]
            for m in meals
        ):
            out.append(
                Conflict(
