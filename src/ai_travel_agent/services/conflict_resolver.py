...
class ConflictResolver:
    ...
    def resolve_all(
        ...
        for c in conflicts:
            if c.severity == Severity.NEEDS_USER:
                ...
                continue
            day = next(
                (d for d in itinerary.days if d.day_number == c.day_number), None
            )
    ...
    def _fix_time_overlap(self, day: DayPlan, c: Conflict, itin: Itinerary) -> None:
        ...
        self._log(
            day.day_number, c.conflict_type, f"Shifted '{b.title}' to {b.start_time}"
        )

    def _fix_impossible_travel(
        self, day: DayPlan, c: Conflict, itin: Itinerary
    ) -> None:
        by_id = {a.attraction_id: a for a in day.activities}
        a, b = by_id.get(c.activity_ids[0]), by_id.get(c.activity_ids[1])
        if not (a and b) or b.locked:
        ...
        self._log(
            day.day_number, c.conflict_type, f"Pushed '{b.title}' to {b.start_time}"
        )

    def _fix_opening_hours(self, day: DayPlan, c: Conflict, itin: Itinerary) -> None:
        act = next(
            (a for a in day.activities if a.attraction_id in c.activity_ids), None
        )
        if not act:
            return
        ...
        self._log(
            day.day_number, c.conflict_type, f"Clamped '{act.title}' to opening hours"
        )
    ...
    def _fix_meal_gap(self, day: DayPlan, c: Conflict, itin: Itinerary) -> None:
        ...
        placeholder = ItineraryActivity(
            ...
            latitude=day.hotel_latitude,
            longitude=day.hotel_longitude,
        )
        day.activities.append(placeholder)
        self._log(
            day.day_number, c.conflict_type, f"Inserted {placeholder.title} at {slot}"
        )

    def _fix_budget_overrun_day(
        self, day: DayPlan, c: Conflict, itin: Itinerary
    ) -> None:
        candidates = [
            a
    ...
    def _duration(self, a: ItineraryActivity) -> int:
        ...
        return int(
            (
                datetime.combine(datetime.today(), a.end_time)
                - datetime.combine(datetime.today(), a.start_time)
            ).total_seconds()
            / 60
        )
    ...
    def _add(t: time, minutes: int) -> time:
        return (
            datetime.combine(datetime.today(), t) + timedelta(minutes=minutes)
        ).time()
