...
class TestPlanAndStatus:
    def test_plan_returns_session_and_job_id(self):
        with patch(
            "ai_travel_agent.api.main._graph.invoke", return_value=FAKE_GRAPH_RESULT
        ):
            r = client.post(
                "/plan", json={"destination": "Paris", "days": 5, "budget": 1500}
            )
            assert r.status_code == 200
            body = r.json()
    ...
    def test_status_completed_after_background_task(self):
        with patch(
            ...
            r = client.post(
                "/plan", json={"destination": "Tokyo", "days": 3, "budget": 2000}
            )
    ...
    def test_plan_graph_failure_marks_job_failed(self):
        with patch(
            "ai_travel_agent.api.main._graph.invoke", side_effect=RuntimeError("boom")
        ):
            r = client.post(
                "/plan", json={"destination": "Nowhere", "days": 1, "budget": 100}
            )
            job_id = r.json()["job_id"]
            status = client.get(f"/status/{job_id}")
...
class TestRefine:
    def test_refine_unknown_session_404(self):
        r = client.post(
            "/refine", json={"session_id": "nope", "instruction": "less walking"}
        )
        assert r.status_code == 404
    ...
    def test_refine_known_session_ok(self):
        with patch(
            ...
            plan = client.post(
                "/plan", json={"destination": "Rome", "days": 4, "budget": 1800}
            )
            session_id = plan.json()["session_id"]
            r = client.post(
                "/refine", json={"session_id": session_id, "instruction": "add museums"}
            )
            assert r.status_code == 200
...
class TestExport:
    def _planned_session(self) -> str:
        with patch(
            ...
            plan = client.post(
                "/plan", json={"destination": "Kyoto", "days": 3, "budget": 1200}
            )
...
class TestFlights:
    def test_search_flights_mocked(self):
        with patch(
            "ai_travel_agent.api.main.flight_tool._run",
...
class TestHotels:
    def test_search_hotels_mocked(self):
        with patch(
            "ai_travel_agent.api.main.hotel_tool._run",
            return_value=[{"name": "Mock Hotel"}],
        ):
            r = client.get(
                "/api/hotels",
                params={
...
class TestAttractions:
    def test_attractions_mocked(self):
        with patch(
            "ai_travel_agent.api.main._attraction_tool._run",
...
class TestRestaurants:
    def test_restaurants_mocked(self):
        with patch(
            "ai_travel_agent.api.main._restaurant_tool._run",
...
class TestWeather:
    def test_weather_mocked(self):
        with patch(
            "ai_travel_agent.api.main._weather_tool._run",
...
class TestBudget:
    def test_update_budget_mocked(self):
        with patch(
            "ai_travel_agent.api.main._budget_tool._run",
...
class TestLegacyEndpoints:
    ...
    def test_legacy_plan_success(self):
        with patch(
            "ai_travel_agent.api.main._run_graph", return_value=FAKE_GRAPH_RESULT
        ):
    ...
    def test_legacy_evaluate(self):
        with patch(
            "ai_travel_agent.api.main.evaluate_itinerary", return_value={"score": 8.5}
        ):
            r = client.post(
                "/api/trip/evaluate", json={"itinerary": {}, "request": "test"}
            )
            assert r.status_code == 200
